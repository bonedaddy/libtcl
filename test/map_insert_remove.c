...
int main(void)
{
	...
	for (i = 0; i < data_sz; i++) {
		ASSERT_TRUE(map_contains(&map, data[i].key));
		ASSERT_TRUE(map_remove(&map, data[i].key));
		ASSERT_FALSE(map_contains(&map, data[i].key));
		ASSERT_FALSE(map_remove(&map, data[i].key));
		ASSERT_EQ(data_sz - i - 1, map_length(&map));
	}
