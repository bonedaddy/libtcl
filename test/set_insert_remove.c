...
int main(void)
{
	...
	for (i = 0; i < data_sz; i++) {
		ASSERT_TRUE(set_contains(&set, data[i].key));
		ASSERT_TRUE(set_remove(&set, data[i].key));
		ASSERT_FALSE(set_contains(&set, data[i].key));
		ASSERT_FALSE(set_remove(&set, data[i].key));
		ASSERT_EQ(data_sz - i - 1, set_length(&set));
	}
