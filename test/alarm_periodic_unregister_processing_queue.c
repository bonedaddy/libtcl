...
// using that queue.
void test_periodic_unregister_processing_queue(void) {
	alarm_t *alarms[5];
	blocking_queue_t queue;
	thread_t thread;

	blocking_queue_init(&queue, UINT32_MAX);
	thread_init(&thread, "timers.test_periodic_unregister_processing_queue.thread");
...
// Schedule each of the timers with different period
	for (int i = 0; i < 5; i++) {
		alarm_set_on_queue(alarms[i], (period_ms_t)20 + i, cb, INT_TO_PTR(i), &queue);
	}
...
int main(void) {
	...
	test_periodic_unregister_processing_queue();
	return (0);
}